...
class Project extends Model implements GloballySearchable, HasGlobalSearchVisibility
{
    ...
    public function globalSearchVisibility(): VisibilityRule
    {
        return VisibilityRule::make()
            ->policy('view')
            ->using(fn ($record, $user) => $user->current_team_id === $record->team_id);
    }
...
<Aside type="tip">
  Override `globalSearchSubtitle()` to provide a custom subtitle instead of relying on auto-discovery.
</Aside>
